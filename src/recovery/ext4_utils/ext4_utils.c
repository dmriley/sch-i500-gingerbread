...
// djp952: added integer result, non-zero on a critical error
int ext4_create_fs_aux_info()
{
...
// djp952: added integer return value; non-zero indicates critical error
int ext4_fill_in_sb()
{
	unsigned int i;
	int result;
	...
	for (i = 0; i < aux_info.groups; i++) {
		...
		if (ext4_bg_has_super_block(i)) {
			if (i != 0) {
				result = queue_data_block((u8 *)sb, info.block_size, group_start_block);
				if(result != 0) return result;
				result = queue_data_block((u8 *)aux_info.bg_desc,
					aux_info.bg_desc_blocks * info.block_size,
					group_start_block + 1);
				if(result != 0) return result;
			}
...
void ext4_create_resize_inode()
{
	...
	if (inode == NULL) {
		warn("failed to get resize inode\n");
		return;
	}
	...
	result = inode_attach_resize(inode, reserve_inode_alloc);
	if(result != 0) return;
...
void ext4_create_journal_inode()
{
	...
	u8 *journal_data = inode_allocate_data_extents(inode,
			...
			info.journal_blocks * info.block_size);
	if (!journal_data) {
		warn("failed to allocate extents for journal data\n");
		return;
	}
