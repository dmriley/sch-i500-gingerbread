...
// djp952: Maybe this won't work for all volume types, but as the
// above comment indicates, we need to skip special file types
// otherwise make_ext4 pretty much just croaks when it tries to
// call error() about it
/*
...
*/
static int filter_dot_and_special_files(const struct dirent *d)
{
	if(strcmp(d->d_name, "..") == 0) return 0;
	if(strcmp(d->d_name, ".") == 0) return 0;
	if(d->d_type == DT_CHR) return 0;
	if(d->d_type == DT_BLK) return 0;
	if(d->d_type == DT_FIFO) return 0;
	if(d->d_type == DT_SOCK) return 0;
	if(d->d_type == DT_UNKNOWN) return 0;
		...
		u32 dir_inode, int android)
{
	...
	// djp952: use filter that omits special files as well
	//entries = scandir(full_path, &namelist, filter_dot, (void*)alphasort);
	entries = scandir(full_path, &namelist, filter_dot_and_special_files, (void*)alphasort);
	if (entries < 0) {
		error_errno("scandir\n");
...
#else
			warn("can't set android permissions - built without android support\n");
#endif
		}
                ...
                char *mountpoint, int android, int gzip, int sparse)
{
	...
	//info("Creating filesystem with parameters:\n");
	//info("    Size: %llu\n", info.len);
	//info("    Block size: %d\n", info.block_size);
	//info("    Blocks per group: %d\n", info.blocks_per_group);
	//info("    Inodes per group: %d\n", info.inodes_per_group);
	//info("    Inode size: %d\n", info.inode_size);
	//info("    Journal blocks: %d\n", info.journal_blocks);
	//info("    Label: %s\n", info.label);

	result = ext4_create_fs_aux_info();
	if(result != 0) return result;

	//info("    Blocks: %llu\n", aux_info.len_blocks);
	//info("    Block groups: %d\n", aux_info.groups);
	//info("    Reserved block group size: %d\n", aux_info.bg_desc_reserve_blocks);

	result = block_allocator_init();
	if(result != 0) return result;

	result = ext4_fill_in_sb();
	if(result != 0) return result;

	if (reserve_inodes(0, 10) == EXT4_ALLOCATE_FAILED)
		warn("failed to reserve first 10 inodes\n");
	...
	//info("Created filesystem with %d/%d inodes and %d/%d blocks\n",
