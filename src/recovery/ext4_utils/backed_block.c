...
int queue_data_block(u8 *data, u32 len, u32 block)
{
...
// djp952: added integer return value; non-zero indicates a critical error
int queue_data_file(const char *filename, off_t offset, u32 len,
	u32 block)
{
	struct data_block *db = malloc(sizeof(struct data_block));
	if (db == NULL) {
	...
	data_block_file_callback_t file_func, struct output_file *out)
{
	...
	u32 last_block = 0;
	u32 total_blocks = 0;
	u32 blocks_processed = 0;
	
	// Count the total number of blocks so we can provide progress
	for(db = data_blocks; db; db = db->next) total_blocks++;

	for (db = data_blocks; db; db = db->next) {
		if (db->block < last_block)
			error("data blocks out of order: %u < %u\n", db->block, last_block);
		...
		blocks_processed++;
		setprogress((float)(blocks_processed * 100) / (float)total_blocks);
	}
}
