...
// djp952: added integer return value; non-zero indicates failure
static int allocate_bg_inode_table(struct block_group_info *bg)
{
	int result;
	...
	return queue_data_block(bg->inode_table, aux_info.inode_table_blocks
...
static int reserve_blocks(struct block_group_info *bg, u32 start, u32 num)
{
	...
	for (i = 0; i < num && block % 8 != 0; i++, block++) {
		if (bitmap_set_bit(bg->block_bitmap, block)) {
			warn("attempted to reserve already reserved block\n");
			return -1;
		}
	}
...
// djp952: Added integer return value; non-zero indicates a critical error
static int init_bg(struct block_group_info *bg, unsigned int i)
{
	...
	result = queue_data_block(bg->bitmaps, 2 * info.block_size, block);
...
int block_allocator_init()
{
	...
	for (i = 0; i < aux_info.groups; i++) {
		result = init_bg(&aux_info.bgs[i], i);
		if(result != 0) return result;
	}
	...
	for (i = 0; i < aux_info.groups; i++) {
		if (aux_info.bgs[i].data_blocks_used == 0) {
			...
			if (len <= bg_len) {
				...
				if (block == EXT4_ALLOCATE_FAILED) {
					warn("failed to allocate %d blocks in block group %d\n", bg_len, i);
					return NULL;
				}
...
struct ext4_inode *get_inode(u32 inode)
{
	...
	result = allocate_bg_inode_table(&aux_info.bgs[bg]);
	if(result == 0) return (struct ext4_inode *)(aux_info.bgs[bg].inode_table + inode * info.inode_size);
	else return NULL;
}
